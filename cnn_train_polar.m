function [net, info,imdb] = cnn_train_polar(net, imdb, getBatch, varargin)
...
if start >= 1
  ...
  if opts.usePolar
  %load([opts.expDir,sprintf('/cents%d.mat',start)],'saveCentHist');
  imdb.images.centerHist = saveLoadCenter(opts.expDir,[],start,false);
  %imdb.images.centerHist = saveCentHist;
  end
end
...
for epoch=start+1:opts.numEpochs
  ...
  val = opts.val ;
  if numGpus <= 1
    [net,stats.train,imdb] = process_epoch(opts, getBatch, epoch, train, learningRate, imdb, net) ;
    [~,stats.val,imdb] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net) ;
  else
    spmd(numGpus)
      [net_, stats_train_,imdb] = process_epoch(opts, getBatch, epoch, train, learningRate, imdb, net) ;
      [~, stats_val_,imdb] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net_) ;
    end
    net = net_{1} ;
  ...
  %if ~evaluateMode && opts.usePolar , save([opts.expDir '/' sprintf('cents%d.mat',epoch)], 'saveCentHist') ; end 
  if ~evaluateMode && opts.usePolar , saveLoadCenter(opts.expDir,imdb.images.centerHist,epoch,true); ; end 
  figure(1) ; clf ;
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,imdb,prof] = process_epoch(opts, getBatch, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
centers = -1;
isFliped = false;
...
stats = [] ;
start = tic ;
H =comm.gpu.AWGNChannel('NoiseMethod','Variance','Variance',1);
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if centers == -1
        centers = -ones(numel(batch),1);
    end
    if opts.usePolar && opts.useGmm
    imdb = updateSigmas(imdb,batch,net);
    end
    %CentHist is of size 2*2*3*batchSize
    %polHist is of size 32*32*3*3*batchSize which is has polarcoor of the
    %centers in centhist
    % orIm is the original images
    [im, labels,CentHist,imdb,isFliped] = getBatch(imdb, batch,net,opts.usePolar,opts.useGmm) ;
    ...
    if opts.prefetch
      ...
      getBatch(imdb, nextBatch,net,opts.usePolar) ;
    end

    if numGpus >= 1 && ~isa(im,'gpuArray')
      im = gpuArray(im) ;
    end

    % evaluate CNN
    net.layers{end}.class = labels ;
    net.layers{1}.centers = CentHist;
    if training, dzdy = one; else,
        dzdy = [] ; end
   
    %if ~opts.useGmm && ~training 
   % netwithoutloss = net;
    %netwithoutloss.layers{end} = struct(type)
   % end
    if epoch >100 && opts.usePolar
    [net,imdb,res_c] = vl_simplenn_polar_updateCenter(net,im,1,isFliped,imdb,batch,true,res_c,opts,s);
    end
  ...
  if training
      if opts.SmoothGrad
      total_net = 20;
      res = create_smooth_gradients( total_net,net,2*learningRate,H,res,s,opts,im,dzdy );
      end
    if numGpus <= 1
